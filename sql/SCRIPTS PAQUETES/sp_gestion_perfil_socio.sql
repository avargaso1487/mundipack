CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gestion_perfil_socio`(IN `opcion` VARCHAR(30), IN `p_usuario` VARCHAR(8), IN `p_contacto` VARCHAR(30), IN `p_atencion` VARCHAR(30), IN `p_password` VARCHAR(300), IN `p_foto_perfil` TEXT, IN `p_foto_carta` TEXT)
    NO SQL
BEGIN

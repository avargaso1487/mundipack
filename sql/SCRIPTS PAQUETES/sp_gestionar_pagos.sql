CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gestionar_pagos`(IN `opcion` VARCHAR(300), IN `p_operacion` VARCHAR(300), IN `p_monto` DOUBLE, IN `p_fecha` DATE, IN `p_pagoID` INT, IN `p_usuario` INT)
    NO SQL
BEGIN

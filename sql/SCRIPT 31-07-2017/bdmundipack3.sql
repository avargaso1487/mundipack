...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 01-08-2017 a las 04:22:54
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_control_viajero_recomentado` (IN `ve_opcion` VARCHAR(300), IN `ve_viajeroNombre` VARCHAR(300), IN `viajeroApellidos` VARCHAR(300), IN `ve_viajeroDNI` VARCHAR(300), IN `ve_viajeroDireccion` VARCHAR(300), IN `ve_viajeroNacimiento` VARCHAR(300), IN `ve_viajeroTelefonoFijo` VARCHAR(300), IN `ve_viajeroTelefonoCelular` VARCHAR(300), IN `ve_viajeroEmail` VARCHAR(300), IN `ve_viajeroNroPasaporte` VARCHAR(300), IN `ve_viajeroAbierto` BOOLEAN, IN `ve_usuario` INT)  NO SQL
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gestionar_notificaciones` (IN `opcion` VARCHAR(30))  NO SQL
BEGIN
...
BEGIN
	IF opcion = 'opc_registrar_pago' THEN
        ...
        ELSE
			SET @PAQUETE = (SELECT Paquete FROM se_viajeropaquetesposibles WHERE Viajero = @VIAJERO AND Prioridad = 1 AND Estado = 1);
			INSERT INTO se_pagocuotaviajero (Viajero, NroOperacion, MontoCuota, FechaPago, Estado, Paquete) VALUES (@VIAJERO, p_operacion, p_monto, p_fecha, 0, @PAQUETE);
    ...
    IF opcion = 'opc_registrar_pago_traveler_admin' THEN
        ...
        IF @VALIDAOPERACION > 0 THEN
			SELECT '0' AS respuesta;
        ELSE
			SET @PAQUETE = (SELECT Paquete FROM se_viajeropaquetesposibles WHERE Viajero = p_pagoID AND Prioridad = 1 AND Estado = 1);
			INSERT INTO se_pagocuotaviajero (Viajero, NroOperacion, MontoCuota, FechaPago, Estado, Paquete) VALUES (p_pagoID, p_operacion, p_monto, p_fecha, 1, @Paquete);
            SELECT 'Se registro correctamente el pago realizado' AS respuesta;
        END IF;
	END IF; 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_gestionar_paquetes` (IN `opcion` VARCHAR(300), IN `p_nombre` VARCHAR(300), IN `p_descripcion` TEXT, IN `p_preciomin` DOUBLE, IN `p_preciomax` DOUBLE, IN `p_precioprom` DOUBLE, IN `p_paqueteID` INT)  NO SQL
BEGIN
	IF opcion = 'opc_registrar_paquete' THEN
		INSERT se_paquetes (Nombre, Descripcion, PrecioMinimo, PrecioMaximo, PrecioPromedio, Estado) VALUES (p_nombre, p_descripcion, p_preciomin, p_preciomax, p_precioprom, 1);
...
BEGIN
    ...
    IF opcion = 'opc_aceptar_transaccion' THEN
        ...
        IF (@VALIDARCONTADOR = 0) THEN
			...
			SET @MONTORETORNO = (cast(@IMPORTE as decimal(8,2))*(cast(@PORCRETORNO as decimal(8,2)))/100);					SET @RETORNOMUNDI = (cast(@IMPORTE as decimal(8,2))*(cast(@COMISION as decimal(8,2)))/100);	 				SET @MONTOVIAJERO = (@MONTORETORNO - @RETORNOMUNDI);				
...
INSERT INTO `se_contadormundi` (`ContadorMundi`, `MontoAcumulado`, `Estado`) VALUES
(2, '4.00', 1);
...
INSERT INTO `se_contadorsocio` (`ContadorSocio`, `Socio`, `MontoAcumulado`, `Estado`) VALUES
(2, 1, '176.00', 1);
...
(6, 5, 'Depositos Viajeros', 1, 'fa fa-paper-plane-o', '../administrador/pagos_traveler.php', b'1'),
...
(18, NULL, 'Travelers', 4, 'fa fa-user', NULL, b'1'),
...
(23, 18, 'Travelers Abiertos', 1, 'fa fa-user', '../travelers/traveler_abiertos.php', b'1'),
(24, 18, 'Travelers Recomendados', 2, 'fa fa-user', '../travelers/traveler_recomendados.php', b'1');
...
(12, 5, 5, NULL, '2017-07-31', 1),
(11, 6, 3, NULL, '2017-07-31', 1),
(10, 5, 4, NULL, '2017-07-31', 1),
(9, 1, 3, NULL, '2017-07-30', 1),
(8, 9, 1, NULL, '2017-07-28', 1);
...
INSERT INTO `se_pagocuotasocio` (`PagoCuotaSocio`, `Socio`, `NroOperacion`, `Monto`, `FechaPago`, `Estado`) VALUES
(1, 1, '75675', 100, '2017-07-24', 0);
...
--
-- Volcado de datos para la tabla `se_pagocuotaviajero`
--

INSERT INTO `se_pagocuotaviajero` (`PagoCuotaViajero`, `Viajero`, `NroOperacion`, `MontoCuota`, `FechaPago`, `Estado`, `Paquete`) VALUES
(2, 1, '1234', 100, '2017-07-23', 1, 1),
(3, 1, '64565', 100, '2017-07-24', 0, 1),
(4, 1, '78686', 100, '2017-07-25', 0, 1);
...
CREATE TABLE `se_paquetes` (
  ...
  `PrecioMaximo` double NOT NULL,
  `PrecioPromedio` double NOT NULL,
  `Estado` bit(1) NOT NULL
) ENGINE=MyISAM DEFAULT CHARSET=latin1;

--
-- Volcado de datos para la tabla `se_paquetes`
--

INSERT INTO `se_paquetes` (`Paquete`, `Nombre`, `Descripcion`, `PrecioMinimo`, `PrecioMaximo`, `PrecioPromedio`, `Estado`) VALUES
(1, 'Paquete 01', 'Incluye Ticket aÃ©reo + Traslado aeropuerto + Hotel + 3 Noches de alojamiento con sistema todo incluÃ­do + Impuestos Hoteleros', 120, 200, 160, b'1'),
(2, 'Paquete 02', 'Incluye Ticket aÃ©reo + Traslado aeropuerto + Hotel + 3 Noches de alojamiento con sistema todo incluÃ­do + Impuestos Hoteleros', 150, 300, 225, b'1'),
(3, 'Paquete 03', 'Incluye Ticket aÃ©reo + Traslado aeropuerto + Hotel + 3 Noches de alojamiento con sistema todo incluÃ­do + Impuestos Hoteleros', 120, 300, 210, b'1');
...
(1, 'MUNDIPACK', '', '00000000', '', '2017-01-01', '000000000', '000000000', 'mundipack@gmail.com', 1),
(2, 'Jorge Luis', 'Arias Tandaypan', '47934182', 'Ramon Castilla 263', '1993-09-09', '044414514', '953536749', 'aristandaypan@gmail.com', 1),
(3, 'Karla Melissa', 'Polo Piminchumo', '47934181', 'sdfsdfdgfsdg', '1993-09-09', '044345682', '12345678', 'karla@gmail.com', 1),
(4, 'PRUEBAAAAA', 'PRUEBAAAAA', '35647684', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 1),
(5, 'PRUEBAAAAA', 'PRUEBAAAAA', '35647684', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 1),
(6, 'PRUEBAAAAA', 'PRUEBAAAAA', '35647684', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 1),
(7, 'PRUEBAAAAA', 'PRUEBAAAAA', '11111111', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 1),
(8, 'PRUEBAAAAA', 'PRUEBAAAAA', '11211111', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 1),
(9, 'PRUEBAAAAA', 'PRUEBAAAAA', '13211111', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 0),
(10, 'PRUEBAAAAA', 'PRUEBAAAAA', '13251111', 'PRUEBAAAAAPRUEBAAAAA', '1993-09-09', '433254253', '534543253', 'aristandaypan@gmail.com', 2),
(11, 'TYERYT', 'YTRYERYTR', '64566356', 'YHFGH', '1993-09-09', '456546', '456546', 'karla@gmail.com', 2);
...
(1, 'OFERTA PRUEBA 1', '2017-07-24', '2017-07-26', '', '2017-07-24', 1, '../../view/partners/ofertas/4754082d-63b2-4192-bc8f-e5fdac5e0315.jpg', b'1', '1');
...
INSERT INTO `se_socio` (`Socio`, `RazonSocial`, `NombreComercial`, `Rubro`, `RUC`, `Direccion`, `TelefonoContacto`, `TelefonoAtencion`, `Email`, `NroCuenta`, `ContactoResponsable`, `PorcentajeRetorno`, `PrecioDesde`, `DiaPago`, `Estado`, `CartaPresentacion`, `Categoria`) VALUES
(1, 'Chuchurros S.A.C', 'Chuchurritos', 1, '12345678912', 'Empresa dedicada a Restaurant', '044414514', '004231779', 'ariastandaypan@gmail.com', '12345635654', 'Jorge Luis Arias Tandaypan', 12, 100, 2, b'1', 'view/partners/carta/1478967_608804732519192_772491443_n.jpg', NULL),
(2, 'Juanitas ERL', 'Juanitas', 1, '1231234545', 'pruebitaaaaa', '044413526', '044238759', 'juanita@gmail.com', '534365634', 'Juana', 12, 20, 2, b'1', NULL, NULL),
(3, '', '', 0, '', '', '', '', '', '', '', 0, 0, NULL, b'1', NULL, NULL);
...
(1, '3.17', '3.31', '2017-07-27', b'0'),
(2, '3.20', '3.21', '2017-07-28', b'1');
...
(6, '0004', '1242343', 1, 1, 200, '2017-07-28', 1, 2),
(5, '2345', '2524325', 1, 2, 100, '2017-07-26', 0, 1);
...
INSERT INTO `se_transaccioncontadormundi` (`TransaccionContadorMundi`, `ContadorMundi`, `Transaccion`, `Comision`, `Estado`) VALUES
(1, 1, 4, 1, 1),
(2, 1, 1, 1, 1),
(3, 1, 2, 1, 1),
(4, 2, 6, 1, 1);
...
INSERT INTO `se_transaccioncontadorsocio` (`TransaccionContadorSocio`, `ContadorSocio`, `Transaccion`, `PorcentajeRetorno`, `Estado`) VALUES
(1, 1, 4, '12.00', 1),
(2, 1, 1, '12.00', 1),
(3, 1, 2, '12.00', 1),
(4, 2, 6, '12.00', 1);
...
(1, 1, NULL, 'admin', 'e10adc3949ba59abbe56e057f20f883e', '2017-04-14 00:00:00', 'view/default/assets/images/users/admin_profile.png', NULL, 1),
(2, NULL, 1, '12345678912', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-23 00:00:00', 'view/default/assets/images/users/12509501_1011414545591540_273207986451627878_n.jpg', NULL, 1),
(3, NULL, 2, '1231234545', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-23 00:00:00', 'view/default/assets/images/users/netpartner_default_profile.jpg', NULL, 1),
(4, 2, NULL, '47934182', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-23 00:00:00', 'view/default/assets/images/users/traveler_default_profile.png', NULL, 1),
(5, 3, NULL, '47934181', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-23 00:00:00', 'view/default/assets/images/users/traveler_default_profile.png', NULL, 1),
(6, NULL, 3, '', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-30 00:00:00', 'view/default/assets/images/users/netpartner_default_profile.jpg', NULL, 1),
(7, 10, NULL, '13251111', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-31 00:00:00', 'view/default/assets/images/users/traveler_default_profile.png', NULL, 2),
(8, 11, NULL, '64566356', 'e10adc3949ba59abbe56e057f20f883e', '2017-07-31 00:00:00', 'view/default/assets/images/users/traveler_default_profile.png', NULL, 2);
...
CREATE TABLE `se_viajero` (
  ...
  `ViajeroRegistro` int(11) DEFAULT NULL
) ENGINE=MyISAM DEFAULT CHARSET=latin1;
...
INSERT INTO `se_viajero` (`Viajero`, `Persona`, `NroPasaporte`, `ViajeroAbierto`, `FechaPago`, `FechaRegistro`, `ViajeroRegistro`) VALUES
(1, 2, '534435436546546', b'1', 2, NULL, NULL),
(2, 3, '5344354365465', b'1', 2, NULL, NULL),
(3, 9, '1234567', b'1', NULL, '2017-07-31', 1),
(4, 10, '1234567', b'1', NULL, '2017-07-31', 1),
(5, 11, '464365465', b'1', NULL, '2017-07-31', 1);
...
--
-- Volcado de datos para la tabla `se_viajeropaquetecomprado`
--

INSERT INTO `se_viajeropaquetecomprado` (`ViajeroPaqueteComprado`, `Paquete`, `Viajero`, `ViajeroPaquetesPosibles`, `Monto`, `Fecha`) VALUES
(1, 1, 1, 1, 160, '2017-07-25');
...
--
-- Volcado de datos para la tabla `se_viajeropaquetesposibles`
--

INSERT INTO `se_viajeropaquetesposibles` (`ViajeroPaquetesPosibles`, `Viajero`, `Paquete`, `Prioridad`, `Estado`) VALUES
(1, 1, 1, 1, 1),
(2, 1, 2, 2, 0);
...
--
ALTER TABLE `se_comision`
  MODIFY `Comision` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
...
--
ALTER TABLE `se_contadormundi`
  MODIFY `ContadorMundi` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
--
-- AUTO_INCREMENT de la tabla `se_contadorsocio`
--
ALTER TABLE `se_contadorsocio`
  MODIFY `ContadorSocio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
--
...
--
ALTER TABLE `se_menu`
  MODIFY `Menu` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=25;
--
-- AUTO_INCREMENT de la tabla `se_movimiento`
--
ALTER TABLE `se_movimiento`
  MODIFY `Movimiento` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=13;
--
-- AUTO_INCREMENT de la tabla `se_pagocuotasocio`
--
ALTER TABLE `se_pagocuotasocio`
  MODIFY `PagoCuotaSocio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
-- AUTO_INCREMENT de la tabla `se_pagocuotaviajero`
--
ALTER TABLE `se_pagocuotaviajero`
  MODIFY `PagoCuotaViajero` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
--
...
--
ALTER TABLE `se_paquetes`
  MODIFY `Paquete` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
--
-- AUTO_INCREMENT de la tabla `se_permiso`
--
ALTER TABLE `se_permiso`
  MODIFY `Permiso` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=33;
--
-- AUTO_INCREMENT de la tabla `se_persona`
--
ALTER TABLE `se_persona`
  MODIFY `Persona` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;
--
...
--
ALTER TABLE `se_rolusuario`
  MODIFY `RolUsuario` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=9;
--
...
--
ALTER TABLE `se_socio`
  MODIFY `Socio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=4;
--
-- AUTO_INCREMENT de la tabla `se_tipocambio`
--
ALTER TABLE `se_tipocambio`
  MODIFY `TipoCambio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;
--
...
--
ALTER TABLE `se_transaccioncontador`
  MODIFY `TransaccionContador` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
--
-- AUTO_INCREMENT de la tabla `se_transaccioncontadormundi`
--
ALTER TABLE `se_transaccioncontadormundi`
  MODIFY `TransaccionContadorMundi` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
--
-- AUTO_INCREMENT de la tabla `se_transaccioncontadorsocio`
--
ALTER TABLE `se_transaccioncontadorsocio`
  MODIFY `TransaccionContadorSocio` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
--
...
--
ALTER TABLE `se_viajero`
  MODIFY `Viajero` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;
--
-- AUTO_INCREMENT de la tabla `se_viajeropaquetecomprado`
--
ALTER TABLE `se_viajeropaquetecomprado`
  MODIFY `ViajeroPaqueteComprado` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;
--
-- AUTO_INCREMENT de la tabla `se_viajeropaquetesposibles`
--
ALTER TABLE `se_viajeropaquetesposibles`
  MODIFY `ViajeroPaquetesPosibles` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
